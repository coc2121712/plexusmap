...
export function ContactActions({ professional }: ContactActionsProps) {
  ...
  return (
    <div className="flex flex-col gap-2 w-full sm:w-auto">
      {/* Primary CTA — WhatsApp (green, prominent) */}
      {whatsappUrl ? (
        <a
          href={whatsappUrl}
          target="_blank"
          rel="noopener noreferrer"
          className="flex items-center justify-center gap-2 bg-green-600 text-white font-medium text-sm px-6 py-3 rounded-xl hover:bg-green-700 transition-colors w-full"
        >
      ...
      {/* No contact info — prompt claim */}
      {!professional.phone && !professional.kairosEnabled && !professional.isClaimed && (
        <div className="bg-gray-50 border border-gray-200 rounded-xl p-3 text-center">
          ...
          >
            ¿Eres este profesional? Reclama tu perfil
          </a>
        </div>
      )}
